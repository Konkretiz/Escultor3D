...
/*
Métodos não finalizados para o projeto da terceira unidade, modificados para tentar atender à unidade 2.
*/

/*
Método para criar matriz 3d alocada dinamicamente com padrão _nx, _ny e _nz = 10, que será o tamanho máximo da "escultura" em seus eixos.
*/
...
Sculptor::Sculptor(int _nx, int _ny, int _nz) : r(0.5), g(0.5), b(0.5), a(0.5)
{
    ...
    if (_nx <= 0 || _ny <= 0 || _nz <= 0) std::cout << "Dimensões inválidas, valores padrão implementados\n";
    if (_nx <= 0)
        _nx = 2;
    if (_ny <= 0)
        _ny = 2;
    ...
    if (!(v && v[0] && v[0][0])) {
        std::cerr << "Impossível alocar espaço.\n";
        std::exit(1);
    }
    ...
    std::cout << "Espaço alocado com sucesso.\n";
}
